...
#define HAL_GYRO_ACCEL_DATA_SIZE                     6
...
bool HalGyroRead(uint8 *pABuf, uint8 *pBuf)
{
  bool success;
  ...
  uint8 accelConfig=0x11;
  HalSensorWriteReg(0x1C, &accelConfig, sizeof(accelConfig));
  // Wait for measurement ready (appx. 1.45 ms)
  ST_HAL_DELAY(180);
  // Read sensor
  success = HalSensorReadReg(HAL_GYRO_REG_Accel_XOUT_H,buf,HAL_GYRO_ACCEL_DATA_SIZE);
